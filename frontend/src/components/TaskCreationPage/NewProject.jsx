...
export default function NewProject({ onAdd }) {
    ...
    function handleSave() {
        ...
        // Validation
        if (!enteredTitle || !enteredDesc || !enteredDueDate || !selectedPriority) {
            modal.current.open();
            return;
        }
        ...
        <>
            ...
            <div className="w-[35rem] mt-16">
                <menu className="flex items-center justify-end gap-4 my-4">
                    <button className="text-stone-400 hover:text-white rounded-md px-4 py-2 transition-all ease-in-out duration-300">
                        Cancel
                    </button>
                    <button 
                ...
                <div>
                    ...
                    <Input title="Priority" type="radio" options={["High", "Medium", "Low"]} name="priority" />
                </div>
            </div>
        </>
    );
